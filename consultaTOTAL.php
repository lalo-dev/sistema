...
function imprimir(valor){	//Función para abrir reporte	
		var win = new Window({className: "mac_os_x", title: "Reporte de Confirmaciones Pendientes", top:70, left:100, width:1200, height:500, url: "scripts/reporteGuiastotal.php?tipo="+valor, showEffectOptions: {duration:.5}}); 
		win.show(); 
}
</script>
...
<?php
	...
	if($tipoReporte==0) //Se deberán tomar los datos de reporte normal
	{
		$sqlConsulta="SELECT cguias.cveGuia,IFNULL(cconsignatarios.estacion,'') AS estacion,IFNULL(cguias.cveLineaArea,'') AS cveLineaArea,".
			 ...
			 "IFNULL(cestados.nombre,'') AS estadoCon,".
			 "IFNULL(cmunicipios.nombre,'') AS municipioCon,".
	...
	//Ciudad
	$destinatarioDesde=$_POST['txtMunicipioD'];
	$destinatarioHasta=$_POST['txtMunicipioH'];
	
	if(($municipioDesde!="" || $municipioHasta!="") && ($municipioDesde!="Desde" || $municipioHasta!="Hasta")){	
		if(($municipioDesde=="") || ($municipioDesde=="Desde"))
		{  $municipioDesde = $bd->minMax("MIN(nombre)",'cmunicipios');	  } 
		
		if(($destinatarioHasta=="") || ($destinatarioHasta=="Hasta"))
		{  $destinatarioHasta = $bd->minMax("MAX(nombre)",'cmunicipios');	  }
		
		$condicion.="AND (cmunicipios.nombre BETWEEN '".$municipioDesde."' AND '".$municipioHasta."') ";		
	}
	 ...
	 {	
		...
		<table width="4444" border="1"> 
			<tr id="Encabezados">
				...
				<th width="460">PLANIFICAC&Oacute;N / FOLIO</th> 
