...
function cancelar()
{
    ...
    //Limpiar cajas, deshabilitar, y quitar clase de inválido
...
function llenaFactura(datos)
{
	...
	//Consultamos los datos de las guías involucradas en la factura
...
{
	//Primero verificar que de quitar ese renglon la factura NO quede vacía
	...
	{
		//Preguntar si se desea eliminar la guía
		...
		{
			//Eliminar la guía de la factura y de la tabla
				...
				$Ajax(url,{onfinish: function(res)
									 {
										...
										resultado=res.split("-");
										alert(resultado[0]);
										//Se compelto la operación exitosamente
										if(resultado[1]==1)
											tr.remove();
...
function datosGuia()
{
   	//Antes de agregar la guía, mostraremos la información de la guía		
...
//Se guardará el valor anterior a el cambio
var valAnterior='';
...
function llenaDatos(campos){
	...
	//Primero verificar que no se repita la guía
	guias=$$('[id^=txtGuia_]');
	...
	{
		if(indice==1)	//No existe la tarifa para realizar los cálculos
		{
...
function verFactura()
{
	...
	var win = new Window({className: "mac_os_x",title:"Relación de guías.",top:70,left:160,width:950,height:500,url:url,showEffectOptions:{duration:1.5}}); 
	win.show(); 
}
