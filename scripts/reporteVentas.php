<?php
	...
	$clientes = $bd->Execute("SELECT DISTINCT(cvecliente) AS cveCliente2,LEFT(razonSocial,24) AS razonSocial FROM ccliente ".
							 "WHERE cveEmpresa=1 AND cveSucursal=1 AND estatus=1 ".
							 "ORDER BY (SELECT IFNULL(SUM(IF(kg>volumen,kg,volumen)),0) AS peso  ".
							 "FROM cguias WHERE cveCliente=cveCliente2 AND estatus=1 AND recepcionCYE  BETWEEN '".$fechaInicio."' AND '".$fechaFin."') DESC,razonSocial ASC;");
	$j=0;
	$a=0;
	...
	foreach ($clientes as $cliente)
	{
		...
		$reporte[$j][0]=$cliente['cveCliente2'];
		$razonSocial[$j]=$cliente['razonSocial'];        
		...
		for($i=1;$i<=$contador;$i++)		
		{
			...
			$sql="SELECT IFNULL(SUM(IF(kg>volumen,kg,volumen)),0) AS peso FROM cguias ".
				 "WHERE cveCliente='".$cliente['cveCliente2']."' AND estatus=1 AND recepcionCYE='$tmpFecha'";
			...
			$datosCliente[$j]=$datosCliente[$j]+$pesoTotal;
			$fechas[$i-1]=$tmpFecha; 
	...
	for($i=0;$i<=$contador;$i++)
	{
		if($datosDias[$i]!=0)
		{
			$porcentajePesoDia=$datosDias[$i]*100/$pesoCompleto;
			$porcentajeDia[$i]=redondear_dos_decimal($porcentajePesoDia);
		}else 
		{
			$porcentajeDia[$i]=0;
			$datosDias[$i]=0;
	...
	for($i=0;$i<$j;$i++)
	{
		if($datosCliente[$i]!=0)
		{
			$porcentajeCliente[$i]= $datosCliente[$i]*100/$pesoCompleto;
	...
	$graph->img->SetMargin(60,10,50,300);
	$graph->title->Set("");

	for($i=0;$i<count($razonSocial);$i++){
		$valor=quitarAcentos($razonSocial[$i]);
		$razonSocial2[$i]=$valor;
	}

	$graph->xaxis->SetTickLabels($razonSocial2);
	...
	$pdf->SetXY(4,$y);
	$pdf->SetTextColor(255);
	$pdf->Cell(40, 4,'PORCENTAJE', 1, 0, 'C', 1);
	$pdf->SetTextColor(0);
	...
	$pdf->Cell(14, 4,ceil($final).' % ' , 1, 0, 'C', 0);
