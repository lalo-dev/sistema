...
function existe() {
		...
		$("rcye").className = "";
		$("txtCName").className = "";
		$("lstDestBranch").className = "";
		$("txtCsnName").className = "";
		$("txtCsnZC").className = "";
		$("txtVol").className = "";
		$("txtPieces").className = "";
		$("txtKg").className = "";
		$("txtValidity").className = "";
		$("lstStatus").className = "";
...
//esta funcion recibe el valor de la función anterior y lo evalúa	
	function next_existe(ex){
		...
		if(allgood()){
		var fechaVuelo = $("txtFlDate").value.substring(6,10)+"-"+$("txtFlDate").value.substring(3,5)+"-"+$("txtFlDate").value.substring(0,2);
		var fechaRcye = $("rcye").value.substring(6,10)+"-"+$("rcye").value.substring(3,5)+"-"+$("rcye").value.substring(0,2);
		var fechaVigencia = $("txtValidity").value.substring(6,10)+"-"+$("txtValidity").value.substring(3,5)+"-"+$("txtValidity").value.substring(0,2);
		var fechaEntrega = $("txtDlvdDate").value.substring(6,10)+"-"+$("txtDlvdDate").value.substring(3,5)+"-"+$("txtDlvdDate").value.substring(0,2);
		var fechaAcuse = $("txtRcptDate").value.substring(6,10)+"-"+$("txtRcptDate").value.substring(3,5)+"-"+$("txtRcptDate").value.substring(0,2);
              var valores = "wb_id="		+$("txtWbId").value		+"&wb_airline="		+$("lstAirLine").value		+"&wb_flightnum="	+$("txtFlNbr").value;
	valores = valores + "&wb_airwb="	+$("txtAirWb").value		+"&wb_flightdate="	+fechaVuelo			+"&wb_rcye="		+fechaRcye;
...
valores = valores + "&wb_kg="			+$("txtKg").value		+"&wb_vol="		+$("txtVol").value		+"&wb_validity="	+fechaVigencia;
valores = valores + "&wb_destbranch="		+$("lstDestBranch").value	+"&wb_destzp="		+$("txtCsnZC").value		+"&wb_destadd="		+$("txtCsnAdd").value;
valores = valores + "&wb_status="		+$("lstStatus").value		+"&wb_delivdate="	+fechaEntrega			+"&wb_receivedby="	+$("txtRcbdBy").value;
valores = valores + "&wb_dlvnbr="		+$("txtDlvNbr").value		+"&wb_vlnbr="		+$("txtVlNbr").value		+"&wb_invnbr="		+$("txtInvNbr").value	+"&wb_receiptdate="	+fechaAcuse		+"&wb_rmks="		+$("txtRmks").value;
...
valores = valores + "&wb_dlvnbr="		+$("txtDlvNbr").value		+"&wb_vlnbr="		+$("txtVlNbr").value		+"&wb_invnbr="		+$("txtInvNbr").value	+"&wb_receiptdate="	+fechaAcuse		+"&wb_rmks="		+$("txtRmks").value;
		...
		$("txtFlDate").value = campo.wb_flightdate.substring(8,10)+"-"+campo.wb_flightdate.substring(5,7)+"-"+campo.wb_flightdate.substring(0,4);
		$("rcye").value = campo.wb_rcye.substring(8,10)+"-"+campo.wb_rcye.substring(5,7)+"-"+campo.wb_rcye.substring(0,4);
		...
		$("txtValidity").value = campo.wb_validity.substring(8,10)+"-"+campo.wb_validity.substring(5,7)+"-"+campo.wb_validity.substring(0,4);
		...
		$("txtDlvdDate").value = campo.wb_delivdate.substring(8,10)+"-"+campo.wb_delivdate.substring(5,7)+"-"+campo.wb_delivdate.substring(0,4);
		...
		$("txtRcptDate").value = campo.wb_receiptdate.substring(8,10)+"-"+campo.wb_receiptdate.substring(5,7)+"-"+campo.wb_receiptdate.substring(0,4);
		...
		if($("rcye").value.length < 10){$("rcye").className += " invalid"; notGood ++;} else{$("rcye").className = "";}
		...
		//if($("txtValidity").value.length < 10){$("txtValidity").className += " invalid"; notGood ++;} else{$("txtValidity").className = "";}
